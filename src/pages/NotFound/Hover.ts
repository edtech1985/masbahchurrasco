...
export const StyledOuterButton = styled.div`
  position: relative;
  margin: 0 0.75rem;
  background: var(--orange); //cor2 de fundo luminous botão
  height: 4rem;
  width: 15rem;
...
export const Span = styled.span`
  ...
  height: 100%;
  width: 100%;
  background: var(--red); //cor1 de fundo luminous botão
  border-radius: 50px;
...
export const HoverableSpan = styled(Span)`
  ...
  ${Button}:hover + & {
    ...
    /* background: linear-gradient(var(--orange), var(--red), var(--brown)); */
    animation: rotate 1s linear infinite;
  }
`;
